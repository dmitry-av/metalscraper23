...
def parse_section(filename):
    try:
            ...
            for item_size, url in sizes.items():
                while True:
                    try:
                        ...
                        # Handle region chooser if present
                        try:
        ...
        driver.quit()
        print(f"Section completed: {section_name}")

    except Exception as e:
        print(f"Error occurred while processing section: {section_name}")
        print(e)
...
def main():
    try:
        ...
        # Initialize proxy pool and driver
        driver = get_driver(proxypool=proxy_pool, visible="yes")
